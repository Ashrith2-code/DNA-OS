...
import {InteractionData, UserState} from './types';
...
export async function* streamAppContent(
  ...
  const userStatusPromptSegment = `Current User Status: ${
    user.isSignedIn ? `Signed in as '${user.name}'` : 'Not signed in'
  }.`;

  const fullPrompt = `${systemPrompt}

${userStatusPromptSegment}
...
Generate the HTML content for the window's content area only:`;
  ...
  try {
    const response = await ai.models.generateContentStream({
      ...
      contents: fullPrompt,
      config: {
        // enabled thinking for higher quality responses
      },
    });
